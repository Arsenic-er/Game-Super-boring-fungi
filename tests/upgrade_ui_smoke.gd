...
func _run() -> void:
	...
	if filter_rects.size() != 8 or (filter_rects.back()["rect"] as Rect2).end.x >= game._minimap_rect().position.x:
		push_error("UPGRADE_UI_FAIL: eight unit filters should fit between the resource bar and minimap")
		quit(1)
		return
	...
	if panel.size.x < 700.0 or panel.size.y < 450.0:
		...
		quit(1)
		return
	print("UPGRADE_UI_OK panel=", panel, " tabs=5 menus=session-rendered barracks_queue=rendered rally=rendered filters=8 rival_fungus=rendered discovery_banner=rendered ecology_event=rendered sporefall=rendered offline_report=rendered chapter_flow=rendered goal_pages=4 expedition_units=2 dish_zoom=", game.camera_zoom)
	game.queue_free()
	quit(0)
